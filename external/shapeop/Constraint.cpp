...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE PlaneCollisionConstraint::PlaneCollisionConstraint(const std::vector<int> &idI,
                                                                  Scalar weight,
                                                                  const Matrix3X &/*positions*/,
                                                                  const Vector3 &ppos,
                                                                  const Vector3 &pnor,
                                                                  const Scalar dist) :
  Constraint(idI, weight), planePoint_(ppos), planeNormal_(pnor),dist_(dist) {
  assert(idI.size() == 1);
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void PlaneCollisionConstraint::project(const Matrix3X & positions, Matrix3X &projections) const {
    Vector3 x = positions.col(idI_[0]) - planePoint_;
    Scalar dist = x.dot(planeNormal_);

//    int nSim = positions.cols()/3;
//    if(idI_[0] == 7032 + 2*nSim)
//    {
//        std::cout << "dist: " << dist << std::endl;
//    }
    if(dist > -dist_)
        projections.col(idO_) = weight_*(positions.col(idI_[0]) - (dist + dist_)*planeNormal_);
    else
        projections.col(idO_) = weight_*positions.col(idI_[0]);
    // if(dist < 0.0)
    //     projections.col(idO_) = weight_*(positions.col(idI_[0]) + (std::fabs(dist) + dist_)*planeNormal_);
    // else
    //     projections.col(idO_) = weight_*positions.col(idI_[0]);
}
///////////////////////////////////////////////////////////////////////////////
SHAPEOP_INLINE void PlaneCollisionConstraint::addConstraint(std::vector<Triplet> &triplets, int &idO) const {
  idO_ = idO;
  triplets.push_back(Triplet(idO_, idI_[0], weight_));
  idO += 1;
}
