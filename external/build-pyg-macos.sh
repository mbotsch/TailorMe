...
CMAKE_PREFIX_DIR=$(realpath "$HOME/miniconda3/envs/spirals/python3.10/site-packages/torch")

#CONDA_EXE=${CONDA_EXE:="mamba"}
...
sed -i.bak "s/set(CMAKE_CXX_STANDARD 14)/set(CMAKE_CXX_STANDARD 17)/g" pytorch_scatter/CMakeLists.txt
sed -i.bak "s/set(CMAKE_CXX_STANDARD 14)/set(CMAKE_CXX_STANDARD 17)/g" pytorch_sparse/CMakeLists.txt

# build options
if [ -z "$TORCH_BUILD_OPTS" ]; then
  TORCH_BUILD_OPTS="-DWITH_CUDA:BOOL=OFF"
  echo "OPTIONS ARE ------------------- $TORCH_BUILD_OPTS"
fi
